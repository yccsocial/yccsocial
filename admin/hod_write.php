...
exit();
  ...
  $result =  $post -> selectUser("teacher","email", $email, $pass);
    ...
    $major_id = $result[0]['department_id'];
    $coordinator = $result[0]['coordinator'];
      ...
      <div class="d-flex">
        <div class="p-3 flex-grow">
          <div class="bg-white border shadow-sm d-inline-block px-4 text-center floated-navigate">
            ...
            <a href="hod_write.php?id=<?php echo $token; ?>"><span class="fas fa-plus fa-lg d-block my-5 active" data-toggle="tooltip" data-placement="right" title="Add Post"></span></a>
            ...
            <a href="hod_user.php?id=<?php echo $token; ?>"><span class="fas fa-users fa-lg d-block my-5" data-toggle="tooltip" data-placement="right" title="Users"></span></a>
            <hr>
        ...
        <div class="p-3 flex-grow-1">
          <div class="d-flex align-items-center">
            ...
            <button class="btn btn-theme rounded-pill shadow-sm post-upload"><span class="fas fa-paper-plane mr-2"></span>Upload Post</button>
          </div>
          <div class="row">
            <div class="col-12 my-3">
              <div class="card p-5 shadow-sm write">
                <form id="postDataForm" action="">
                <input class="form-control rounded-pill title-of-post" type="text" placeholder="Enter post title">
                  ...
                  <div class="d-flex flex-row mb-4 align-items-center my-4">
                    ...
                    <select class="custom-select rounded-pill post-class " name="class" multiple='multiple'  style="width: 150px;"> 
                      <?php
                       if($position != 'major_hod'){
                        if($coordinator!=""){
                            $coordinator_result = $post->selectClassesID($id);
                                $coordinator_id = implode(', ', $coordinator_result); 
                                echo "<option value='$coordinator_id'>$coordinator(coordinator)</option>";
                            
                        } 
                        if($family_id!=""){
                            $family_result = $post -> selectFamily($family_id);
                            $family_class = $family_result[0];
                            echo "<option value='$family_id'>$family_class(family)</option>";
                        }
                        ...
                        echo "<option value='$teach_id'>$teach_class</option>";
                        }
                    }   else{
                
                        $teach_classes =$post->selectClassByMajor($major_id);
                        for($i=0;$i<count($teach_classes);$i++){
                            $teach_id =$teach_classes[$i]['id'];
                            $teach_class = $teach_classes[$i]['class_name'];
                        echo "<option value='$teach_id'>$teach_class</option>";
